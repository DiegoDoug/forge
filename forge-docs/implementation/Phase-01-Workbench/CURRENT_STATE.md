...
**In progress — Milestone 1 (Foundation) started.** Specification is locked ([ADR-0009](../../decisions/0009-phase-specification-freeze.md)). T1 (Vault → Secrets compatibility migration) is complete; T2–T4 have not been started. Work is on branch `feature/t1-vault-secrets-migration`.
...
- [ ] TODO: nothing in progress yet — T2 (dedicated `/search` page) is next per the milestone ordering notes in `09_IMPLEMENTATION_TASKS.md` §2.

## Remaining

- [ ] T2–T16 in [`09_IMPLEMENTATION_TASKS.md`](09_IMPLEMENTATION_TASKS.md) — the rest of Milestone 1 (Foundation), then Milestones 2–4.
...
- [x] `forge-docs/implementation/Phase-01-Workbench/09_IMPLEMENTATION_TASKS.md` (T1 checked off)

## Next Milestone

Milestone 1 — Foundation (T1–T4): Secrets compatibility migration (done), `/search` page, `workbench_layout` migration, the panel contract. See [`IMPLEMENT.md`](IMPLEMENT.md) "Milestone Plan" and [`09_IMPLEMENTATION_TASKS.md`](09_IMPLEMENTATION_TASKS.md).

## Next Claude Prompt

```
You are working in the Forge repository as a Claude Code session, on branch
feature/t1-vault-secrets-migration.
...
T1 (Vault → Secrets compatibility migration) is complete. Begin work on: T2 in
09_IMPLEMENTATION_TASKS.md (the dedicated /search page), the second task of
...
- 2026-07-21 — **T1 complete** (branch `feature/t1-vault-secrets-migration`). Vault → Secrets compatibility migration executed per ADR-0006: database tables were already correctly named, so the work was module/route/UI renames plus two temporary aliases (`/api/vault` proxy, frontend `/vault` redirect). See "Completed" above for the full file list. Milestone 1 is not yet checkpoint-complete — T2–T4 remain.
