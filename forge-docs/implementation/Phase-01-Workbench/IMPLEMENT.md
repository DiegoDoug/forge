...
Give Forge a configurable home workspace that surfaces pinned tools, recent activity across every feature, and quick actions, replacing the current fixed Dashboard.

Evolves the existing **Dashboard** feature (`frontend/features/dashboard`, `backend/app/dashboard.py`) into a customizable workspace rather than replacing it outright.
